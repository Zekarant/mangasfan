<div class="container-fluid contenu">
	<div class="row">
		...
		<div class="col-lg-10">
			<h2 class="titre">Modifier "<?= \Rewritting::sanitize($jeu['name_jeu']) ?>"</h2>
			<a href="../" class="btn btn-sm btn-outline-info">Retourner à l'index de la rédaction</a>
			<a href="../../../jeux-video/<?= \Rewritting::sanitize($jeu['slug']) ?>" target="_blank" class="btn btn-sm btn-outline-info">Accéder au jeu</a>
			<hr>
			<div id="navigation">
				<center>
			...
			<div id="en-tete" class="bloc_contenu">
				...
				<form method="POST" action="">
					<div class="row">
						...
						<div class="col-lg-9">
							<input type="text" class="form-control" name="title_game" value="<?= \Rewritting::sanitize($jeu['name_jeu']);?>" /><br/>
						</div>
					</div>
					<div class="row">
						<div class="col-lg-3">
							<label>Modifier l'image sur l'accueil des jeux : <strong>(Environ 320*480 pixels)</strong></label><br/>
						</div>
						<div class="col-lg-9">
							<input type="text" class="form-control" name="picture_game" value="<?= \Rewritting::sanitize($jeu['banniere_jeu']);?>" />
							<small><a href="<?= \Rewritting::sanitize($jeu['banniere_jeu']) ?>" target="_blank">Voir l'image (Nouvel onglet)</a></small>
						</div>
					</div>
					<br/>
					<div class="row">
						...
						<div class="col-lg-9">
							<input type="text" class="form-control" name="picture_pres" value="<?= \Rewritting::sanitize($jeu['cover_jeu']);?>" />
			...
			<div id="presentation" class="bloc_contenu">
				<form method="POST" action="">
					...
					<textarea name="text_pres" class="form-control"  rows="10" cols="70" placeholder="Votre commentaire" ><?= \Rewritting::sanitize($jeu['description_jeu']);?></textarea>
					<hr>
							...
							<?php foreach($recupererArticles as $articles) {
								if($articles['id_onglet'] == $onglets['id_category']){ ?> 
									<tr>
										...
										<td><a href="../modifierArticleJeu.php?id=<?= \Rewritting::sanitize($articles['id_article']) ?>&jeux=<?= \Rewritting::sanitize($articles['id_jeux']) ?>" class="btn btn-outline-info">Modifier la page</a></td>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("span.modif_cat").on("click",function(){
			...
			if(taille >= 0) {
				if(taille < 3 || taille > 12){
					...
					$.ajax({
						url : "../categories.php?id_jeu=" + id_jeu + "&page_id=" + ($("span.modif_cat").index(this) + 1) + "&new_name=" + nouveau_nom,
						type : 'GET',
						success : function(data){
							window.location.reload();
							console.log("../categories.php?id_jeu=" + id_jeu + "&page_id=" + ($("span.modif_cat").index(this) + 1) + "&new_name=" + nouveau_nom);
						}
					});
				}
			}
		...
		$("span.suppr_cat").on("click",function(){
			...
			if(can_suppr){
				$.ajax({
					url : "../supprimerCategories.php?id_jeu=" + id_jeu + "&page_id=" + ($("span.suppr_cat").index(this) + 1),
					type : 'GET',
					success : function(data){
						window.location.reload();
					}
		...
		$("span.suppr_page").on("click",function(){
			...
			if(can_suppr){
				$.ajax({
					url : "../supprimerPage.php?id_jeu=" + id_jeu + "&page_id=" + (num_elt_page + 1),
					type : 'GET',
					success : function(data){
						window.location.reload();
					}
