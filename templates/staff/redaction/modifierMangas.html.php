<div class="container-fluid contenu">
	<div class="row">
		...
		<div class="col-lg-10">
			<h2 class="titre">Modifier "<?= \Rewritting::sanitize($manga['titre']) ?>"</h2>
			<a href="../" class="btn btn-sm btn-outline-info">Retourner à l'index de la rédaction</a>
			<a href="../../../mangas/<?= \Rewritting::sanitize($manga['slug']) ?>" target="_blank" class="btn btn-sm btn-outline-info">Accéder au manga</a>
			<hr>
			<div id="navigation">
				<center>
			...
			<div id="en-tete" class="bloc_contenu">
				...
				<form method="POST" action="">
					<div class="row">
						<div class="col-lg-3">
							<label>Modifier le titre du manga :</label>
						</div>
						<div class="col-lg-9">
							<input type="text" class="form-control" name="title_game" value="<?= \Rewritting::sanitize($manga['titre']);?>" /><br/>
						</div>
					</div>
					<div class="row">
						<div class="col-lg-3">
							<label>Modifier l'image sur l'accueil des mangas : <strong>(Environ 320*480 pixels)</strong></label><br/>
						</div>
						<div class="col-lg-9">
							<input type="text" class="form-control" name="picture_game" value="<?= \Rewritting::sanitize($manga['banniere']);?>" />
							<small><a href="<?= \Rewritting::sanitize($manga['banniere']) ?>" target="_blank">Voir l'image (Nouvel onglet)</a></small>
						</div>
					</div>
					<br/>
					<div class="row">
						...
						<div class="col-lg-9">
							<input type="text" class="form-control" name="picture_pres" value="<?= \Rewritting::sanitize($manga['cover']);?>" />
							<small><a href="<?= \Rewritting::sanitize($manga['cover']) ?>" target="_blank">Voir l'image (Nouvel onglet)</a></small>
						</div>
					</div>
					<br/>
					<div class="row">
						<div class="col-lg-3">
							<label>Type du manga : </label>
						</div>
						<div class="col-lg-9">
					...
					<hr>
					<input type="hidden" id="id_news" class="<?= $manga['id'];?>">
			...
			<div id="presentation" class="bloc_contenu">
				<form method="POST" action="">
					<label for="text_presentation">Modifier la présentation du manga : </label>
							...
							<?php foreach($recupererArticles as $articles) {
								if($articles['id_onglet'] == $onglets['id_category']){ ?> 
									<tr>
										...
										<td><a href="../modifierArticleMangas.php?id=<?= $articles['id_article'] ?>&manga=<?= $articles['id_anime_mangas'] ?>" class="btn btn-outline-info">Modifier la page</a></td>
										<td><span class="suppr_page btn btn-outline-danger">Supprimer la page</span></td>
										<td><a href="../../../jeux-video/<?= \Rewritting::sanitize($articles['slug']) ?>/<?= \Rewritting::sanitize($articles['slug_article']) ?>" target="_blank" class="btn btn-outline-info">Accéder à l'article</a></td>
									</tr>
								<?php }
							} 
						}?>
			...
			<div id="rediger" class="bloc_contenu">
				...
				<form method="POST" action="">
					...
					<br />
					<label for="modif_image_redac">Modifier l'image du jeu : <span style="font-weight:normal; font-style:italic;">Environ 780*400 pixels</span> <span class="message2"></span></label>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("span.modif_cat").on("click",function(){
			...
			if(taille >= 0) {
				if(taille < 3 || taille > 12){
					...
					$.ajax({
						url : "../categoriesMangas.php?id_manga=" + id_jeu + "&page_id=" + ($("span.modif_cat").index(this) + 1) + "&new_name=" + nouveau_nom,
						type : 'GET',
		...
		$("span.suppr_cat").on("click",function(){
			...
			if(can_suppr){
				$.ajax({
					url : "../supprimerCategoriesMangas.php?id_manga=" + id_jeu + "&page_id=" + ($("span.suppr_cat").index(this) + 1),
					type : 'GET',
					success : function(data){
						window.location.reload();
					}
		...
		$("span.suppr_page").on("click",function(){
			...
			if(can_suppr){
				$.ajax({
					url : "../supprimerPageMangas.php?id_manga=" + id_jeu + "&page_id=" + (num_elt_page + 1),
					type : 'GET',
					success : function(data){
						window.location.reload();
					}
