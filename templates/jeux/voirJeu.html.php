...
<center>
	<?php if ($notes == 0) {
		echo "Il n'y a aucun vote pour ce jeu pour l'instant.";
	} else { ?>
	...
	<div class="row">
			...
			<div id="carouselExampleInterval" class="carousel slide" data-ride="carousel">
				<div class="carousel-inner">
					<div class="carousel-item active" data-interval="10000" style="object-fit: cover;">
						<span class="onglet"><?= \Rewritting::sanitize($articlesJeux[0]['name_category']); ?></span>
						<a href="<?= \Rewritting::sanitize($jeu['slug'])?>/<?= \Rewritting::sanitize($articlesJeux[0]['slug_article']) ?>">
							<img src="<?= \Rewritting::sanitize($articlesJeux[0]['cover_image_article']); ?>" class="d-block w-100" style="height: 25em; object-fit: cover;">
						</a>
						<span class="title_last_art"><?= \Rewritting::sanitize($articlesJeux[0]['name_article']); ?></span>
					</div>
					<?php foreach ($articlesJeux as $page): ?>
						<?php if ($page[0] != $lastArticle[0]) { ?>
							<a href="#">
								<div class="carousel-item">
									<span class="onglet"><?= $page['name_category']; ?></span>
									<a href="<?= \Rewritting::sanitize($jeu['slug'])?>/<?= \Rewritting::sanitize($page['slug_article']) ?>">
		...
		<div class="col-lg-6">
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-4">
						<center><span id="titre" style="text-align:left;" class="<?= \Rewritting::sanitize($jeu['id_jeux']) ?>"><img src="https://zupimages.net/up/18/25/es4a.png" style="padding-right:10px;font-size:21px;"/>Catégories</span></center>
						<hr>
						<div id="onglet">
							<?php $i = 0;
...
<script type="text/javascript">
	$(function() {
		$("span.name_cat,span.cat_active").on("click",function(){
			...
			$.ajax({
				url : "../jeux-video/categories.php?id_elt=" + id_jeu + "&type=" + type + "&name_cat=" + name_cat,
				type: 'GET',
