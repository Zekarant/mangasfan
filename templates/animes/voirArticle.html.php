<div class="d-flex justify-content-left">
	...
	<?php if ($utilisateur['grade'] >= 5) { ?>
		<a href="../../staff/redaction/modification-animes/<?= \Rewritting::sanitize($article['slug']) ?>" style="margin-left: 10px;" class="btn btn-sm btn-outline-info">Accéder à la rédaction de "<?= \Rewritting::sanitize($article['titre']) ?>"</a>
		<a href="../../staff/redaction/modification-animes/<?= \Rewritting::sanitize($article['slug']) ?>/<?= \Rewritting::sanitize($article['slug_article']) ?>" style="margin-left: 10px;" class="btn btn-sm btn-outline-info">Modifier l'article "<?= \Rewritting::sanitize($article['name_article']) ?>"</a>
	<?php } ?>
</div>
<hr>
