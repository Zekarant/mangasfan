...
<html>
	<head>
		...
		<link rel="stylesheet" href="<?= $ok_page; ?>../overlay.css" />
	...
	<body>
		...
		<header>
			...
			<div class="slogan_site"><?= $slogan; ?></div>
		        <?php include("../elements/navigation.php") ?>
			<h2 id="actu_moment"><?= $phrase_actu; ?></h2>
          <h5 id="slogan_actu"><?= $slogan_actu; ?></h5>
		...
		<?php if (!is_null($id_jeu) && !is_null($id_page)){
			...
			if ($verif_jeu_exist->rowCount() > 0 && $verif_page_exist->rowCount() > 0){ 
				...
				<h2 id="titre_news"><?= $verif_page_existe['nom'];?></h2>
				...
				<div class="accueil_jeu">
					...
					<?php if ($taille_liste_pages >= 1) { ?>
						...
						<h5 class="titre_mobile">Retrouvez tous les articles</h5>
							...
							<div id="onglet">
								<center><span id="titre_news" style="text-align:left;" class="<?= $id_jeu ?>"><img src="https://zupimages.net/up/18/25/es4a.png" style="padding-right:10px;font-size:21px;"/><span class="couleur_mangas">C</span>atégories</span></center>
					...
					<p class="direction">Se diriger sur la page :
						<select>
		...
		<?php } else { ?>
			<div id="titre_news">Nos <span class="couleur_mangas">Jeux</span> <span class="couleur_fans">Vidéos</span>
			</div>
			<hr>
		 	...
		 	<div id="conteneur_images_jeux">
				...
				<div class="gallery">
  					<a href="https://www.mangasfan.fr/jeux_video/<?= traduire_nom(stripslashes($donnees['titre'])); ?>">
   						<img src="<?= bbcode($donnees['vignette']); ?>" />
  					</a>
