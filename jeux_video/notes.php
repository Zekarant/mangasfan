	...
	function use_note($pdo,$id_elt,$type_elt){
		...
		if($type_elt == "anime" || $type_elt == "manga" || $type_elt == "jeux"){ // verification non nécessaire
			if (isset($_SESSION['auth'])){
				...
				$vote_pst = $pdo->query("SELECT * FROM note_members WHERE member = '$user->username' AND type_elt = '$type_elt' AND id_elt = '$id_elt'");
				if ($vote_pst->rowCount() == 0){ 
					...
					if(isset($_POST['etoile1'])){
						$vote_ok->execute(array($user->username,1,$type_elt,$id_elt));
						$vote = false;
					} elseif (isset($_POST['etoile2'])){
						$vote_ok->execute(array($user->username,2,$type_elt,$id_elt));
						$vote = false;
					} elseif (isset($_POST['etoile3'])){
						$vote_ok->execute(array($user->username,3,$type_elt,$id_elt));
						$vote = false;
					} elseif (isset($_POST['etoile4'])){
						$vote_ok->execute(array($user->username,4,$type_elt,$id_elt));
						$vote = false;
					} elseif (isset($_POST['etoile5'])){
						$vote_ok->execute(array($user->username,5,$type_elt,$id_elt));
						$vote = false;
					}?>
