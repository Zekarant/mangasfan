...
<head>
	<meta charset="utf-8">
	<title>Modifier le programme d'animation - Mangas'Fan</title>
...
<body>
	...
	elseif (isset($_SESSION['auth']) AND $utilisateur['grade'] < 2) {
		?>
		<div class='alert alert-danger' role='alert'>
			Vous ne pouvez pas accéder à cette page. <a href="../index.php">Retourner sur l'index</a>.
		</div>
					...
					<nav>
						...
						<div class="col-sm-10" style="background-color: white; border-left: 2px solid grey; padding: 0px!important;">
							...
							if (!empty($_POST['valider']) AND isset($_POST['valider'])) {
								if (!empty($_POST['titre']) AND isset($_POST['titre'])) {
									if (!empty($_POST['contenu']) AND isset($_POST['contenu'])) {
										$enregistrement = $pdo->prepare('UPDATE programme_animation SET title = ?, contenu = ? WHERE id = ?');
										$enregistrement->execute(array($_POST['titre'], $_POST['contenu'], $_GET['planning']));
										?>
										<div class="alert alert-success" type="alert">
											Le programme d'animation a bien été modifié <?php echo sanitize($utilisateur['username']); ?> !
										</div>
									<?php } else { ?>
							...
							<section class="marge_page">
								<?php 
								$animation_modifier = $pdo->prepare('SELECT * FROM programme_animation WHERE id = ?');
								$animation_modifier->execute(array($_GET['planning']));
								$modifier = $animation_modifier->fetch();
								?>
								<h3 class="titre_principal_news">
									Modifier le planning « <?php echo sanitize($modifier['title']); ?> »
								</h3>
								<div class="container">
									<div class="row">
										<div class="col-md-12">
											<center><h4>Modifier un programme</h4></center>
											<form method="POST" action="">
												<label>Titre du programme : (Animation de juillet, décembre, d'été)</label>
												<input type="text" name="titre" class="form-control" placeholder="Mettez le titre du programme d'animations" value="<?php echo sanitize($modifier['title']); ?>">
												<br/>
												...
												<textarea name="contenu" class="form-control" placeholder="Mettez le programme d'animations ici">
													<?php echo sanitize($modifier['contenu']); ?>
												</textarea>
												<input type="submit" class="btn btn-sm btn-info" name="valider" value="Modifier le programme">
											</form>
										</div>
									</div>
								</div>
