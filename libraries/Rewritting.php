...
class Rewritting {
  ...
  /**
   * Transforme une chaîne quelconque en chaîne acceptable pour une URL. Utilise le trait d'union comme séparateur.
   * @param string $str La chaîne à traiter.
   * @return string La chaîne traitée.
   */
  public static function stringToURLString(string $str): string {
    $str = self::remove_accents($str);
    $str = preg_replace('`\s+`', ' ', trim(preg_replace('`[^a-z0-9\040\-/]`is', '', iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $str))));
    $str = strtolower($str);
  ...
  public static function name_url($elt){
  // traduit le nom en paramètre pour pouvoir l'appeler dans l'url
  $elt = Rewritting::suppr_accents($elt);
