...
class Forum extends Model {
	...
	public function allSujets(int $idForum, $idUser){
		$add1 = "";
		$add2 = "";
		if ($idUser != 0) {
		...
		$req = $this->pdo->prepare('SELECT f_topics.id_topic, topic_titre, topic_createur, topic_vu, topic_post, topic_posted, topic_last_post,
		Mb.username AS membre_pseudo_createur, Mb.id_user AS id_utilisateur_posteur, date_created, Ma.username AS membre_pseudo_last_posteur, Ma.id_user AS id_utilisateur_derniere_reponse, id_message, '.$add1.' post_id FROM f_topics 
		LEFT JOIN users Mb ON Mb.id_user = f_topics.topic_createur
		LEFT JOIN f_messages ON f_topics.topic_last_post = f_messages.id_message
		LEFT JOIN users Ma ON Ma.id_user = f_messages.id_user
		'.$add2.'
		...
		if ($idUser != 0) $req->bindValue(':id',$idUser);
		$req->execute();
		$sujets = $req->fetchAll();
		return $sujets;
	}
	...
	public function ajouterMessage($idTopic, $utilisateur, $message, $forum){
        ...
        $req = $this->pdo->prepare('UPDATE forum_topic_view SET tv_post_id = 1, tv_poste = :poste WHERE tv_id = :id AND tv_topic_id = :topic');
		$req->execute(['poste' => $nouveaupost, 'id' => $idUser, 'topic' => $idTopic]);
	}
	...
	public function updateVu($idUser, $idTopic, $lastPost){
		$req = $this->pdo->prepare('UPDATE forum_topic_view SET tv_post_id = :last_post WHERE tv_topic_id = :topic AND tv_id = :id');
    	$req->execute(['last_post' => $lastPost, 'topic' => $idTopic, 'id' => $idUser]);
	}
}