...
class Mangas extends Model {
	...
	public function mangaDemande(string $name){
	...
	public function searchManga($id){
	...
	public function verifierNbrArticles(int $mangas){
		$verif_jeu_exist = $this->pdo->prepare("SELECT * FROM mangas_animes WHERE id = :mangas AND type = 'manga' LIMIT 1");
	...
	public function oneManga(int $idManga){
