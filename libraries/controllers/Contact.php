...
class Contact extends Controller {
	...
	public function contact(){
		...
		if (isset($_POST['envoyer'])) {
            $key_secret = '6LcKjKUZAAAAACsz45OTGAWLkM6lnkCsbBKZsj3L';
            ...
            $demandeTextArea = $_POST['demande'];

            if ($response_final->success) {
                if (isset($_POST['pseudo']) AND isset($_POST['email']) AND isset($demandeTextArea) AND !preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $_POST['email']) AND !empty($demandeTextArea)) {
			...
			$variables = array_merge($variables, ['errors', 'couleur', 'demandeTextArea']);
		}
