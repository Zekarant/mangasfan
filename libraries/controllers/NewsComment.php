...
class NewsComment extends Controller {
	...
	public function delete() {
		...
		if (!isset($_SESSION['auth'])) {
			\Http::redirect('../commentaire.php?id=' . \Rewritting::sanitize($_GET['news']));
		}
	...
	public function edit(){
		...
		if (isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])) {
			...
			if ($user['id_user'] != $newsSearch['auteur']) {
				\Http::redirect('../commentaire.php?id=' . \Rewritting::sanitize($newsSearch['id_news']));
			}
			if(isset($_POST['valider'])){
				...
				if (isset($commentary['id_commentary'])) {
					$pageTitle = "Modifier mon commentaire - " . \Rewritting::sanitize($commentary['title']);
