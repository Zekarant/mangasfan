...
$recuperer_recrutements = $pdo->prepare('SELECT * FROM recrutements WHERE link = "moderateurs"');
$recuperer_recrutements->execute();
$redacteurs = $recuperer_recrutements->fetch();
if ($redacteurs['recrutement'] == 0) {
	$_SESSION['flash']['warning'] = '<div class="alert alert-warning">Les recrutements Modérateurs ne sont pas ouverts pour le moment.</div>';
	header('Location: ../');
	exit();
}
if (isset($_POST['envoyer']) && !empty($_POST['envoyer'])) {
	if(isset($_POST['pseudo']) AND isset($_POST['email']) AND isset($_POST['motivations']) AND isset($_POST['experiences']) AND isset($_POST['idees']) AND isset($_POST['disponibilites']) AND !preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $_POST['email'])){
		...
		Mise en situation : " . sanitize($_POST['idees']) . "<br/>
		Motivations : " . sanitize($_POST['motivations']) . "<br/>
		Autre : " . sanitize($_POST['autre']) . "<br/>";
		mail("contact@mangasfan.fr", "Recrutements Modérateurs - " . sanitize($_POST['pseudo']) . "", $demande, $header);
...
<body>
	<div class="bg-recrutements">
		...
		<h1 class="titre-principal-recrutements">Recrutements - Devenez Modérateur !</h1>
	</div>
	<section>
		<h2 class="titre_principal_news">Recrutements - La justice de Mangas'Fan</h2>
		<hr>
		<div class="alert <?= sanitize($redacteurs['color']); ?>">
			<strong>Important :</strong> Sachez qu'en postulant au rôle de modérateur, vous vous engagez à respecter les conditions d'utilisation de Mangas'Fan.
		</div>
					...
					<form method="POST" action="">
						<div class="row">
							...
							<div class="col-md-8">
								...
								<textarea type="text" class="form-control" rows="5" name="idees" placeholder="Un membre insulte un membre dans les commentaires des news. Comment réagissez-vous ? Vous possédez les bannissements, les MP et la suppression de commentaires." required></textarea>
							</div>
						</div>
						<br/>
						<div class="row">
							...
							<div class="col-md-6">
								<label>Avez-vous un ou plusieurs expérience(s) dans la modération : *</label>
