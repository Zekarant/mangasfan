...
if ($redacteurs['recrutement'] == 0) {
	$_SESSION['flash']['warning'] = '<div class="alert alert-warning">Les recrutements Rédacteurs ne sont pas ouverts pour le moment.</div>';
	header('Location: ../');
	exit();
}
if (isset($_POST['envoyer']) && !empty($_POST['envoyer'])) {
	if(isset($_POST['pseudo']) AND isset($_POST['email']) AND isset($_POST['motivations']) AND isset($_POST['experiences']) AND isset($_POST['disponibilites']) AND !preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $_POST['email'])){
		...
		mail("contact@mangasfan.fr", "Recrutements Rédacteurs - " . sanitize($_POST['pseudo']) . "", $demande, $header);
...
<body>
	<div class="bg-recrutements">
		...
		<h1 class="titre-principal-recrutements">Recrutements - Devenez Rédacteur !</h1>
	</div>
	<section>
		<h2 class="titre_principal_news">Recrutements - Les plûmes de Mangas'Fan</h2>
		<hr>
		<div class="alert <?= sanitize($redacteurs['color']); ?>">
			<strong>Important :</strong> Sachez qu'en postulant au rôle de Rédacteur, vous vous engagez à respecter les conditions d'utilisation de Mangas'Fan.
		</div>
						...
						<div class="row">
							...
							<div class="col-md-4">
								...
								<select name="poste" class="form-control">
									<option value="Rédacteur (Anime)">Rédacteur anime</option>
									<option value="Rédacteur (Jeux vidéo)">Rédacteur jeux vidéo</option>
									<option value="Rédacteur (Mangas)">Rédacteur mangas</option>
