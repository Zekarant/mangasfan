...
$recuperer_recrutements = $pdo->prepare('SELECT * FROM recrutements WHERE link = "developpeurs"');
$recuperer_recrutements->execute();
$redacteurs = $recuperer_recrutements->fetch();
if ($redacteurs['recrutement'] == 0) {
	$_SESSION['flash']['warning'] = '<div class="alert alert-warning">Les recrutements Développeurs ne sont pas ouverts pour le moment.</div>';
	header('Location: ../');
	exit();
}
if (isset($_POST['envoyer']) && !empty($_POST['envoyer'])) {
	if(isset($_POST['pseudo']) AND isset($_POST['email']) AND isset($_POST['motivations']) AND isset($_POST['experiences']) AND isset($_POST['disponibilites']) AND isset($_POST['langages']) AND !preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $_POST['email'])){
		...
		Langages maîtrisés : " . sanitize($_POST['langages']) . "<br/>
		...
		mail("contact@mangasfan.fr", "Recrutements Développeurs - " . sanitize($_POST['pseudo']) . "", $demande, $header);
...
<body>
	<div class="bg-recrutements">
		...
		<h1 class="titre-principal-recrutements">Recrutements - Devenez Développeur !</h1>
	</div>
	<section>
		<h2 class="titre_principal_news">Recrutements - La maintenance de Mangas'Fan</h2>
		<hr>
		<div class="alert <?= sanitize($redacteurs['color']); ?>">
			<strong>Important :</strong> Sachez qu'en postulant au rôle de développeur, vous vous engagez à respecter les conditions d'utilisation de Mangas'Fan.
		</div>
				...
				<div class="card-body">
					...
					<form method="POST" action="">
						<div class="row">
							...
							<div class="col-md-4">
								<label>Langage(s) maîtrisé(s) : *</label>
								<input type="email" class="form-control" name="langages" placeholder="Saisir vos langages maitrisés" required>
							</div>
						</div>
						<br/>
						<div class="row">
							...
							<div class="col-md-6">
								<label>Avez-vous un ou plusieurs expérience(s) dans le développement : *</label>
