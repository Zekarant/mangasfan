...
if ($redacteurs['recrutement'] == 0) {
	$_SESSION['flash']['warning'] = '<div class="alert alert-warning">Les recrutements Community Manager ne sont pas ouverts pour le moment.</div>';
	header('Location: ../');
	exit();
}
if (isset($_POST['envoyer']) && !empty($_POST['envoyer'])) {
	if(isset($_POST['pseudo']) AND isset($_POST['email']) AND isset($_POST['motivations']) AND isset($_POST['experiences']) AND isset($_POST['disponibilites']) AND !preg_match("#^[a-z0-9._-]+@(hotmail|live|msn).[a-z]{2,4}$#", $_POST['email'])){
		...
		mail("contact@mangasfan.fr", "Recrutements Community Manager - " . sanitize($_POST['pseudo']) . "", $demande, $header);
...
<body>
	<div class="bg-recrutements">
		...
		<h1 class="titre-principal-recrutements">Recrutements - Devenez Community Manager !</h1>
	</div>
	<section>
		<h2 class="titre_principal_news">Recrutements - La parole de Mangas'Fan</h2>
		<hr>
		<div class="alert <?= sanitize($redacteurs['color']); ?>">
			<strong>Important :</strong> Sachez qu'en postulant au rôle de <?= sanitize($redacteurs['name']); ?>, vous vous engagez à respecter les conditions d'utilisation de Mangas'Fan.
		</div>
