<?php function str_random($length){
...
function sanitize(string $string): string {
  return htmlentities(trim($string), ENT_IGNORE, 'UTF-8');
}

function statut($rang, $sexe)
{
  if ($rang == 1) {
  ...
  if ($sexe == "Homme" || $sexe == "Autre" || $sexe == NULL) {
    if ($rang == 3) {
      ...
      echo '<span class="badge badge-danger">Fondateur</span>';
    }
    else if($rang == 11){
    ...
    if ($rang == 3) {
      echo '<span class="badge badge-warning">Animatrice</span>';
    } else if($rang == 4) {
      echo '<span class="badge badge-community">Community Manager</span>';
    }
    else if($rang == 5){
      echo '<span class="badge badge-redacteur">Newseuse</span>';
    }
    else if($rang == 6){
      echo '<span class="badge badge-redacteur">Rédactrice</span>';
    }
    else if($rang == 7){
      echo '<span class="badge badge-success">Modératrice</span>';
    }
    else if($rang == 8){
      echo '<span class="badge badge-info">Développeuse</span>';
    }
    else if($rang == 9){
      echo '<span class="badge badge-admin">Administratrice</span>';
    }
    else if($rang == 10){
      echo '<span class="badge badge-danger">Fondateur</span>';
    }
...
function traduire_nom($elt){
  ...
  $remplacement = preg_replace('#\'s#','-',strtolower($elt));
  $remplacement = preg_replace('#([\s|:|-|_]+)#i','-',strtolower($remplacement));
  $remplacement = preg_replace('#([!?,.\']+)#i','',strtolower($remplacement));
  $remplacement = rtrim($remplacement,'-');
  return $remplacement;
}
function name_url($elt){
  // traduit le nom en paramètre pour pouvoir l'appeler dans l'url
  $elt = suppr_accents($elt);
