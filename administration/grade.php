      ...
      <!doctype html>
       <html lang="fr">
         <body>
               ...
               <br />
               Bonjour, <br/>
               Si vous recevez cet email, c\'est que nous avons décidé de vous bannir des services de <i>Mangas\'Fan</i>.<br/>
               Voici le motif renseigné par l\'équipe du site : « '.$ban_valide['raison'].' » <br/>Votre bannissement durera jusqu\'au '.$ban_valide['duree'].'. <br/><br/>
			A la suite de cette date, vous serez débanni et aurez de nouveau accès à votre compte en tant que membre ! Les accès au site seront donc de nouveau accessible pour vous.<br/>
			Pour toute question, nous vous invitons à nous tenir au courant afin que de nouvelles mesures soient prises seulement si cela est nécessaire ! <br/><br/>
...
while($don = $sql->fetch())
{
echo"<option value='".$don['id']."' for='membres'>".(utf8_encode($don['username']))." » ".$don['grade']."</option>";
}
?>
       </select></th>
   </p>
 <p>
		<th> 
		 <select name="grades" id="grades" style="width: 100%;" class="form-control">
				 ...
				 <option value='9' for='grades'>Modérateur (9)</option>
				 <option value='10' for='grades'>Développeur (10)</option>
				 <option value='11' for='grades'>Administrateur (11)</option>
		 </select></th>
 </p>
