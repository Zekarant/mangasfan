...
<body>
 ...
 <section class="marge_page">
    <?php
     ...
     <a href="redac.php">Retournez à l'index de la rédaction</a> - <a href="modif_news_mangas.php?id_news=<?php echo $new['billets_id'];?>">Retournez à l'administration du mangas</a>
    <?php 
      $suppression = false;
        if(!empty($_POST['supprimer_page'])){
          $position_onglet = $pdo->query("SELECT * FROM billets_mangas_pages WHERE onglet_id = '$select_all_news['onglet_id']'");
          
          while($position_all = $position_onglet->fetch()){
            if ($position_all->position > $select_all_news['position']){
              $update = $pdo->prepare("UPDATE billets_mangas_pages SET position = ? WHERE id = '$position_all['id']'");
              $update->execute(array($position_all['position'] - 1));
            }
          }

    	   $delete_page_mangas = $pdo->exec("DELETE FROM billets_mangas_pages WHERE id = '$crit'");
         $suppression = true;
