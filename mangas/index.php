...
$id_mangas = isset($_GET['mangas']) ? $_GET['mangas'] : NULL;
...
if((int) $id_mangas == 0 && !is_null($id_mangas)){
	...
	$recup_id = $pdo->query("SELECT id, titre, type FROM billets_mangas WHERE type = 'mangas'");
...
$req = $pdo->prepare('SELECT *, billets_mangas.id AS id_billet, DATE_FORMAT(billets_mangas.date_creation, \'%d %M %Y à %Hh %imin\') AS date_creation_fr FROM billets_mangas WHERE id = ? AND type = "mangas" ORDER BY date_creation');
...
if (isset($_POST['search_ok']) && !empty($_POST['search'])) {
	...
	$recuperer = $pdo->prepare('SELECT * FROM billets_mangas WHERE titre = ? AND type = "mangas"');

	$recuperer->execute(array($_POST['search']));

	$mangas = $recuperer->fetch();

	if (isset($mangas['titre'])) {

		header('Location: ' . traduire_nom($mangas['titre']));

	} else {

		$message = "Ce mangas n'est actuellement pas référencé sur Mangas'Fan !";
...
<body>
		...
		if ($verif_jeu_existe != 0){ 
			...
			<div class="accueil_jeu">
				...
				if(!empty($donnees_jeu['presentation'])){ ?>
					...
					<p><?= htmlspecialchars_decode(bbcode($donnees_jeu['presentation'])); ?></p>
			...
			<?php } else { ?>
				...
				<div class='alert alert-warning' role='alert'>
					...
					Info : Il n'y a pas encore de page pour ce manga.
  ...
  <br/>
		...
		<h1 class="titre_principal_news">Accueil des mangas - Mangas'Fan</h1>
		...
		<div class="conteneur">
			...
			<div class="row">
				...
				<?php $req = $pdo->query("SELECT *, billets_mangas.id AS id_billet, DATE_FORMAT(billets_mangas.date_creation, '%d %M %Y à %Hh %imin') AS date_creation_fr FROM billets_mangas WHERE type = 'mangas' ORDER BY date_creation DESC LIMIT $limit_start, $pagination");
				...
				while ($donnees = $req->fetch()){ ?>
					...
					<div class="col-md-3">
						...
						<div class="card">
							...
							<a href="../mangas/<?= traduire_nom(htmlspecialchars($donnees['titre'])); ?>" target="_blank">
